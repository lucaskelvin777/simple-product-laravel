...
<div class="container">
    ...
    <div class="d-flex justify-content-center">
        <div class="">
            ...
            <h4 class="mb-4"> Editar produto <span class="primary">{{$product->name}}</span></h4>
            <form id="formProduct" action="{{URL('/change')}}" method="post" enctype="multipart/form-data">
                {{csrf_field()}}
                ...
                <div class="row justify-content-center">
                    <figure class="img-preview-card d-flex justify-content-center">
                        <img src="{{empty($product->image) ? URL::asset('/assets/img/no-image.png') : URL::asset('/img/products/'.$product->image) }}" alt="" class="img-fluid img-preview " id="imgPreview">
                ...
                <div class="form-group">
                    ...
                    <input id="name" name="name" value="{{$product->name}}" class="form-control" type="text" placeholder="Nome" required="true" />
                </div>
                <div class="form-group">
                    <label for="description">Descrição</label>
                    <textarea id="description" name="description" class="form-control" type="text" placeholder="Descrição" required="true">{{$product->description}}</textarea>
                </div>
                <div class="d-flex justify-content-center mt-4">
                    <button class="btn btn-primary"><i class="fas fa-check"></i> Editar</button>
                </div>
            </form>
        </div>
